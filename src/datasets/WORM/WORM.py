...
def load_data(img_gt_path, train, multiclass):
    ...
    retain_ind = [i for i, lab in enumerate(labels) if lab in multiclass]
    
    # filter points and labels to only include the ones in the multiclass
    points = [points[i] for i in retain_ind]
    labels = [labels[i] for i in retain_ind]

    # convert label elements from string to indicies
    labels = [multiclass.index(lab) + 1 for lab in labels]
