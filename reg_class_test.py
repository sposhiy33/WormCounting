...
def get_arg_parser():
    ...
    parser.add_argument(
        "--point_weights", type=str, help="Path of model checkpoint of regression model"
    )
    parser.add_argument(
        "--class_weights",
        type=str,
    ...
    parser.add_argument(
        "--hsv", action="store_true", help="whether or not hsv channels are used"
    )
...
def main(args):
    ...
    result = evaluate_crowd_w_fine_grained(
        regr_model=regr_model,
        class_model=class_model,
        data_loader=data_loader_val,
        device=device,
        vis_dir=args.result_dir,
    )
